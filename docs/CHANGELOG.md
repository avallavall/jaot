...
- **Dependencies upgraded to latest stable (2026-06-27)** — Swept backend (`requirements*.txt`) and frontend (`package.json`) to the latest stable releases, validated phase-by-phase against the full test suites (3779 backend pytest, 511 frontend vitest, production build green at each step). Backend majors: anthropic 0.87→0.112, redis 7→8, websockets 14→16, sse-starlette 2→3, prometheus-fastapi-instrumentator 7→8, argon2-cffi 23→25, pypdf 5→6, plus dev tooling (ruff 0.15, mypy 2.1, mutmut 3.6 — config migrated `paths_to_mutate`→`source_paths`). Frontend majors: typescript 5.9→6.0, jsdom 28→29, katex 0.16→0.17, flexsearch 0.7→0.8 (dropped the redundant `@types/flexsearch`; 0.8 ships its own types), plus radix/tiptap/recharts/next-intl/shiki/zustand minors. A small set is intentionally held at a documented cap where the new major changes behaviour: **fastapi** (`<0.137` — router mounting regresses to an 8-route skeleton in 0.137+/0.138.1), **radix-ui** (`<1.6` — 1.6 breaks the singleton popover), **lucide-react** (`<1.0` — 1.x removed brand icons used in social links), and **@vitejs/plugin-react** / **eslint** (peer-incompatible with the current vite / eslint-config-next). recharts 3.9 tightened the Tooltip `Formatter` type (two chart formatters adjusted to infer it).
