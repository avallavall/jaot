...
- **A webhook announcing that a schedule was switched off is sent after it is switched off**, not before, so it can no longer report a change that the server then abandoned. One was the unsubscribe line at the foot of every message, another was the whole "how is it going?" ask two weeks in. Both pointed at pages that never existed.
