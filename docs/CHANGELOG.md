...
- **The comparer's work chart and the home page race said "1 nodes"**. The words follow the count now, in every language.
...
- **Triggered solves ran on the wrong worker, with the wrong solver, outside the limits.** A trigger's solve went to the small worker that sends email and webhooks, with no time limit, always on SCIP whatever solver the model chose, and without the instance's variable cap, time ceiling or daily quota. It now runs like any other solve.
...
- **A secret set in `.env` was shown as configured but never used.** The admin panel said "Configured (env)" for an Anthropic key, an SMTP password or storage keys in `.env`, and the error message suggested putting them there, but the app read them from the database only. A value set in the admin panel still takes priority.
- **Cancelling a CBC or GLPK solve left the solver program running** on the server until its own time limit, up to an hour, slowing down the next solve on that worker. It now stops with the solve.
- **One comparison that hung or crashed could block every comparison on the platform, and stay "running" for good.** A comparison now has a time limit covering all its solvers, a solver that crashes its worker is recorded as failed instead of being run again and again, and a comparison whose worker died is closed. A matrix row cancelled while it was still being prepared now shows its solvers as cancelled, not as failed.
- **A solve waiting behind a long one could be marked "failed, lost task" and then run anyway, with its result thrown away.** A waiting solve is now left alone while its queue still has work in it, a solve that was already stopped is not run, and the status page reports the stop instead of "pending". The page that waits for a result also stops at once when the solve was cancelled, instead of after ten minutes.
...
### Changed
- **The home page says what JAOT does in plain sentences, in all five languages.** The old copy leaned on slogans ("Don't just solve — understand", "AI Agents Meet Optimization", "Honest by design") and on six example results that were never measured ("23% distance reduction", "31% better allocation"). Every section now states what the product does and what the worked examples show; the six use cases describe the decision the model makes instead of quoting an invented saving. The "How it works" steps name the studio, JModel and the Data tab as they exist today, and the tool listing no longer carries a hard-coded count. The marketplace headings, the search-engine descriptions of the marketplace and the author index, and the contact confirmation follow the same style, and the Terms of Service describe the studio instead of a drag-and-drop builder that was retired months ago.
- **The studio and the results screens say things in plain sentences, in all five languages.** The in-app text used the same slogan style the home page shed: a dash standing in for a full stop, closing lines such as "one model, many scenarios" and "what you see is what was measured", and phrasing such as "derivative works are welcome, identical copies are not". Fifty-eight messages were rewritten as short sentences and re-translated into Spanish, Catalan, French and German. They cover the studio subtitles and hints, the JModel and dataset messages, the metric tooltips, the publishing help, the scenario and sensitivity notes, and the solver insights.
- **The rest of the in-app text follows the same plain style.** The glossary definitions, the builder messages, the trigger screens, the workspace role list, the settings, the maintenance banner and the legal pages had the same habit of a dash where a full stop belonged, plus a literal double hyphen that reached the screen as "--". 172 phrasings were corrected across the five languages, and the glossary no longer says a solution is "pressed against" a limit. Page titles keep their dash, which is what a browser tab expects.

### Fixed
- **Seventeen templates were solving with their settings thrown away.** A template card carries settings that tell its generator how to read the data — which column is the cost, which pairings are allowed, whether the goal is throughput or price. The marketplace copy of the card never stored them, and the marketplace copy is what the studio uses. Six of the seventeen failed outright when opened from the studio; the other eleven built a different model and still reported "optimal". A property portfolio ignored its risk ceiling, a warehouse layout could not price a slot at all, and a mine's haulage plan stopped maximising throughput. All seventeen now return the same answer through the studio as they do from the card.
- **Templates now answer the question their card asks.** A model can compile, solve and report "optimal" while reading none of the data it was given: the generator falls back to a hidden default cost of 1 or distance of 100, and every answer ties. Forty-one templates were doing that. All forty-one are repaired. Vehicle routing priced every leg the same, so any tour was optimal; cutting stock cut one of each piece; a portfolio's budget could never bind, so every asset was bought; a vessel that costs 15,000 a day to sail was free; every clinical trial phase lasted one month. A build check now changes each number in a card's example and rebuilds: a number that leaves the model untouched fails the build, unless the card names the field and says why it is context.
- **The train timetable and the tournament fixture list are real models.** Both sat on the shift-rostering generator. The timetable read no preferred departure, no journey time, no priority and no track headway, and the fixture list treated every team as an employee with an hourly cost of 1, so no two teams were ever guaranteed to meet. The timetable now chooses a departure minute per train, works each train's route out from the track segments between its stations, holds the stated headway on any segment two trains share, and shifts the trains where shifting costs least. The fixture list draws a single round robin, picks who hosts each game, keeps every club near half its games at home, and caps how many away games a club plays in a row.
- **The job shop schedules operations instead of staffing shifts.** Its card promised a makespan and a fixed processing order and delivered a shift roster with no durations and no precedence. It now places eleven operations from four orders on three machines, one operation at a time per machine, and finishes in the earliest hour the machine loads and the operation order allow.
- **The studio form sends the whole example.** The form is built from a separate list of fields, and that list had fallen behind the example data on twenty-five cards. Any value with no field behind it was dropped between "Load example" and "Solve". Of the eight cards affected, four then failed outright and four came back "optimal" with a different answer — a mine plan returned 600,000 where its own example means 445,909. Adding a row had the same hole: a row the form builds carries only the columns it knows about, so four cards rejected it and six built a model on defaults.
- **Five cards described a model the platform does not run.** Satellite scheduling promised slew-time and downlink limits, dye batch scheduling promised colour-changeover costs and water usage, fleet dispatch promised truck payloads and cycle times, and the train timetable claimed to read a speed class. None of those exist in the models. Six more cards counted things their data does not have — four fields where there are two, forty students where there are six, a warehouse capacity four times the real one.
- **Every card states the true size of its model.** The variable and constraint counts are served by the API and fed to the assistant, and fifty-five of a hundred and two were wrong. One said 24 variables for a model that builds 1,250.
- **One template had no text in any language but English.** The marketplace and the studio read card text from the translation files, and fall back to English without saying so when a card is missing. The check meant to catch that counted the translation file against a hardcoded number and never looked at the templates, so it stayed green.
- **A train timetable ignored a one-minute safety gap.** A corridor that asks for one minute between trains on a shared track got no gap at all, so two trains could enter the same section in the same minute. Anything above a minute always worked.
...
- **Ten messages explained themselves only in English.** A Spanish, Catalan, French or German user who lacked permission read "You do not have permission to edit", while the English version named the role they hold and the role to ask for. The same gap dropped the reason behind a solver error and behind an invalid form. All four languages now carry the full message.
- **The import confirmation names the model again outside English.** In Spanish, Catalan, French and German it said only that the model had been imported; the English version has always named it.
