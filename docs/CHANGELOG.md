...
- **A solve you start from the studio tells you when it finishes.** Only runs of a marketplace model left a notification; every other path — including the studio, which is where most solves start — showed its toast and left the bell empty. All three now report through the same writer.
