...
### Fixed
- **Losing access to a workspace closes the model instead of leaving the editor open.** Somebody removed from a workspace kept a full workbench on screen — tabs, canvas, an enabled Solve button — while every request behind it was refused, and a reload changed nothing. It now says the model belongs to a workspace you are no longer in, and that what you typed after that was not saved.
