...
- **The bell now rings when a solve finishes.** The notification a completed or failed model run should leave was written and then discarded with the worker's session, on every run since the feature existed — the panel just never had anything to show. Failed runs had a second silencer: naming the model read a field the model does not have. Both delivered now, both kinds.
