...
- **A notification email is only recorded as sent if it was sent.** Author notifications — somebody adopted your model, somebody reviewed it — were never delivered at all: the code marked them sent and returned success without contacting the mail server. They are sent now, and a failure leaves the record honest.
