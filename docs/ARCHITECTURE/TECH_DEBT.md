...
## D-35 · A page that reads its params with React 19's `use()` cannot be rendered in vitest

`frontend/src/app/[locale]/join/[token]/page.tsx` takes `params: Promise<{token}>` and unwraps it
with `use()`. Under vitest it never mounts: four attempts (a Suspense boundary, an awaited act to
flush the promise, both) left the DOM empty, so the page has no render test. Its routing decision
is covered where it lives — `loginPathReturningTo` in `lib/return-path.ts` — but nothing asserts
that the page calls it, or that it prints one error sentence instead of two (`ff8e1bd`).

This is not specific to that page: every route component that unwraps `params` this way has the
same hole, and the App Router hands them all a promise. Worth solving once, in the test setup,
