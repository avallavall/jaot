...
- **The reaper sweeps three tables.** `reap_stale_executions` settles abandoned `ModelExecution` rows against the Celery result backend. A `pending` row whose solver queue still holds messages is left alone, because it may be waiting its turn behind a long solve. `reap_stale_trigger_runs` does the same for `TriggerRun`, because `cron_fire_task` refuses to fire while any run of that trigger looks unfinished — so one abandoned run stops that schedule for good, and every later tick records an ordinary-looking `skipped_overlap`. A `TriggerRun` stores no task id, so that sweep asks the broker which runs a worker is holding rather than judging by age alone. `reap_stale_comparisons` closes a `SolverComparison` whose columns are all settled and whose time budget is spent.
- **No countdown longer than 30 minutes.** A worker holds a delayed message unacknowledged until it is due. RabbitMQ closes a channel that holds one past `consumer_timeout` (30 minutes by default), and the Celery worker then shuts down. Work that is due days later is sent by a beat sweep instead: the onboarding emails after day 0 go out from `send_due_onboarding_emails`, every hour.
