...
flowchart TB
    subgraph Current["CURRENT STATE (Post-Phase 3)"]
        ...
        Monolith["BC2–BC7 in Monolith<br/>app/services/<br/>• Marketplace<br/>• Identity<br/>• AI Assistant<br/>• Automation<br/>• Observability<br/>BC8: Platform Admin in app/shared/<br/>(BC3 Billing REMOVED — ADR-008)"]
    end
    ...
    subgraph Dependencies["Extraction Order (roadmap §6)"]
        Order["1. Solver (done)<br/>2. Observability (pure leaf)<br/>3. AI Assistant (PSS-only)<br/>4. Identity (fan-in 32+)<br/>5. Billing — removed (ADR-008)<br/>6. Marketplace (depends on Identity)<br/>7. Automation (orchestrates the rest)<br/>8. Platform Admin (singleton, do not extract)"]
    end
