...
| **Solver** | The program that searches for the answer: SCIP, HiGHS, CBC, GLPK, JAOS. |
...
**The program that does the searching.** JAOT ships five (SCIP, HiGHS, CBC, GLPK and JAOS) and can be built with a sixth (Hexaly). They all take the same problem and return the same shape of answer, so switching one for another changes the time and sometimes the route, never the meaning.

`solver_name: "auto"` lets the platform pick. Which one it picks, and why it will not pick CBC, GLPK or JAOS for you, is in [what `auto` picks](/docs/api/solve#what-auto-picks).
...
Solving five datasets makes five executions. So does comparing the five solvers.
...
> **Building on JAOT rather than using it?** The repo carries the other half of
> this page: [`docs/GLOSSARY.md`](https://github.com/avallavall/jaot/blob/main/docs/GLOSSARY.md)
> maps every word below onto the class, table and module that implements it.
