...
When a solve returns an optimal or feasible result, JAOT gives you three layers of analysis so you don't just get numbers -- you understand them. All appear on the execution result page.

## 1. Exact, solution-based analysis

The page **leads** with figures computed from your actual solution x* and the problem data -- binding constraints, per-constraint slack and utilization, and which objective terms drive the result. These are exact for the integer solution and work identically on every solver. The full tour of that page -- the grouped structured solution, the solve summary, and the exact analysis -- lives in [Analyzing Results](/docs/studio/analyzing-results).

## 2. Sensitivity analysis (LP-based)

The solver also computes classical sensitivity after solving. It answers: *how sensitive is the optimum to small changes?* On the execution page it sits in a collapsed section under the exact analysis -- useful, but read the caveats below before making a decision from it.
...
## 3. Plain-language explanation (AI)
