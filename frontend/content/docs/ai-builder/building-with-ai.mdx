...
The maximum file size is **50 MB**. Uploaded files are parsed and their content is included as context for the AI, allowing it to extract parameters, understand data structures, and reference specific values from your documents.
...
> **Tip:** Use the [Templates Gallery](/docs/ai-builder/templates-gallery) for common problem types. Templates are pre-built, so no AI formulation step is needed.

> **Modeling in JModel?** The [JModel DSL lens](/docs/studio/jmodel-dsl) has its own **Generate with AI**: it writes an indexed JModel source from a description, a screenshot, or a PDF of a formulation, and every returned source is verified against the compiler before it is marked as valid.
