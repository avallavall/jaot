...
To compare a file you have on disk, parse it first with `POST /api/v2/import/preview` and send the parsed problem as `problem`. The file itself is never stored.
...
**Do not compare `wall_time_ms` between two comparisons.** The seconds mean something inside one comparison, on one machine, run in sequence. `machine_note` records which machine, so a stored table still says where its numbers came from.
