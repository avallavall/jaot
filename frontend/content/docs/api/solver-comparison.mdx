...
Each solver counts as one execution against the instance's daily solve limit. A comparison of 5 solvers costs 5. A matrix of 3 datasets and 5 solvers costs 15. A request that cannot afford all of its runs is rejected whole — half a table invites a conclusion the missing half might have contradicted.

## Which solvers can compare

Pass the names from `GET /api/v2/solvers/available`. The comparable ones are `scip`, `highs`, `cbc`, `glpk` and `jaos`. Hexaly cannot take part: it needs its own container image and licence, and the comparison worker runs from the base image.
...
launch = httpx.post(
    ...
    json={
        ...
        "solver_names": ["scip", "highs", "cbc", "glpk", "jaos"],
...
```json
{
  ...
  "results": [
    ...
    {"solver_name": "jaos", "status": "pending", "solver_status": null}
  ],
  "agreement": null,
