...
| `transaction_type` | string | -- | Filter by type: `execution`, `refund`, `adjustment` (the `purchase`, `sale_earning`, and `withdrawal` types are inert when `MONETIZATION_ENABLED` is off) |
