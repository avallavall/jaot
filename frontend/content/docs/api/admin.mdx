...
## List executions across the platform
...
for run in data["items"]:
    print(f"{run['id']} - {run['organization_name']} - {run['model_name']}")
```

```bash
curl "https://jaot.io/api/v2/admin/executions?status=failed" \
  -H "Authorization: Bearer ok_live_admin_key..."
```

```javascript
const response = await fetch(
  "https://jaot.io/api/v2/admin/executions?status=failed",
  { headers: { "Authorization": "Bearer ok_live_admin_key..." } }
);

const { items, stats } = await response.json();
console.log(`${stats.total} failed runs`);
items.forEach((run) => {
  console.log(`${run.id} - ${run.organization_name} - ${run.model_name}`);
});
```
</CodeTabs>
