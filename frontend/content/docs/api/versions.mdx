---
title: Versions
description: Commit-grade model project versions, diffs, restore — and legacy builder checkpoints
order: 8
---
...
|-------|------|----------|-------------|
| `summary` | string | Yes | What changed (the commit message) |
| `body` | string | No | Why — extended rationale |
...
```bash
curl -X POST "https://jaot.io/api/v2/projects/mp_abc123/commit" \
  -H "Authorization: Bearer ok_live_..." \
  -H "Content-Type: application/json" \
  -d '{"summary": "Tighten capacity to 80"}'
```
</CodeTabs>
...
`POST /api/v2/projects/{project_id}/solve?version_id={version_id}` solves the frozen snapshot instead of the draft — see the [Models API](/docs/api/models).
