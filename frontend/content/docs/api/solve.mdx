...
| `solver_name` | string | No | Solver override: `"scip"`, `"highs"`, `"cbc"`, `"glpk"`, `"hexaly"`, or `"auto"` (default: platform default). See [what `auto` picks](#what-auto-picks). |
...
To pick CBC or GLPK, name one — and use the [solver comparer](/docs/api/solver-comparison)
