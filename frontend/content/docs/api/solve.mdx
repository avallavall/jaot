...
| `solver_name` | string | No | Solver override: `"scip"`, `"highs"`, `"cbc"`, `"glpk"`, `"jaos"`, `"hexaly"`, or `"auto"` (default: platform default). See [what `auto` picks](#what-auto-picks). |
...
- `version` is the solver's own version string. Absent when the solver will not say. Record it
  next to any timing you keep: seconds measured against CBC 2.10.12 explain nothing about 2.11.
...
- Multi-objective is not listed: it is available for every solver. Solvers without
  ...
  `POST /solve/multi-objective` runs on the solver named in `problem.solver_name` (or the
  `solver_name` query parameter), SCIP when none is named, and `auto` picks one. A solver
  that cannot take a quadratic objective is refused with a 422 before anything runs. The
  response carries `execution_id` and `solver_used`.
- `solver_name: "auto"` is always accepted and is not listed here; the server picks the
  effective solver per problem and reports it back as `solver_used`, with `auto_route_reason`
  saying why.
...
**CBC, JAOS and GLPK are substitutes, not candidates.** `auto` never picks them while the
solver its rule prefers is installed. It reaches them only on a server built without that solver,
...
The order is SCIP, HiGHS, CBC, JAOS, GLPK. They sit behind SCIP and HiGHS for reasons that
have nothing to do with speed. CBC and GLPK compute no shadow prices or reduced costs, and JAOS
computes them for an LP only, so a caller who chose no solver could lose the sensitivity
analysis. On a 60-lot burn-in plan (1,342 binaries) HiGHS and CBC finished in 1.5 seconds,
SCIP in 12, and JAOS and GLPK both ran the full 60-second limit without finding a feasible
answer. JAOS comes before GLPK because it returns an LP's shadow prices; GLPK is also
single-threaded.

A substitute is never given a model it cannot express, so a quadratic never reaches CBC,
JAOS or GLPK whatever else is missing.

JAOS is an open-source LP/MIP solver written in C. It is deterministic: the same model gives
the same answer and the same work count on any machine and any thread count. In JAOT it
solves linear and mixed-integer models only. It returns exact shadow prices and reduced costs
for an LP, and none for a MIP.

To pick CBC, GLPK or JAOS, name one — and use the [solver comparer](/docs/api/solver-comparison)
to find out which one is right for your model, with numbers, instead of guessing.
