...
JAOT exposes **26 MCP tools** organized into seven categories:
...
| `activate_catalog_model` | Yes | Activate a marketplace model for your organization (free) |
...
| `solve_model_project` | Yes | Solve a model project's draft or a specific committed version |
...
MCP tool calls that trigger optimization solves draw from your organization's credit quota, following the same usage formula as direct API calls. Use `get_credit_balance` to check your balance, and the `validate_problem` tool to estimate the cost before solving.

See [Rate Limits and Credits](/docs/reference/rate-limits-credits) for the complete credit usage formula.
