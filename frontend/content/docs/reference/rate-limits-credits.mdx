...
## Credits as a Usage Quota

Credits are a free usage quota, not something you buy — monetization is off by default on the hosted instance. A solve draws at least one credit; the quota and per-plan caps are platform settings a self-host operator can raise from the admin panel. See the [Credits API](/docs/api/credits-billing).
