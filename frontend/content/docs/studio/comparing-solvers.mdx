...
The wait is stated before you start, because the solvers run one after another: five solvers with a 60-second limit is up to five minutes, not one.
...
Five charts are drawn under the table, and only when they add something.
...
- **How each solver closed the gap** — one line per solver on one clock, showing what was still left between the answer it held and the bound it had proved. A line that falls steeply found a good answer early; a line that runs flat spent its time proving one it already had; a line that reaches the bottom closed the gap completely. Two solvers can report the same answer in the same total time and get there completely differently, and this is the only place that shows it.
- **How much searching each solver did** — one small panel per solver: the nodes it explored, or the simplex iterations when the model has no tree, against the clock. The time chart says who was slower; this says why. A solver can lose because it explored far more of the tree, or because each node cost it more, and the count beside the seconds is what tells those two apart.
...
**Every panel of the work chart keeps its own vertical scale, on purpose.** A node in one solver is not a node in another: they presolve differently, add different cuts and end up searching a different tree, so putting the counts on one shared axis would invite exactly the comparison the table warns against. What the panels do share is the clock, because a second means the same thing to all of them. A solver that reported only its final count gets a single dot rather than a curve.

**Only SCIP, CBC and JAOS report while they search.** The gap chart needs a running commentary with a clock attached, and that is not something every solver publishes: GLPK prints its progress without any timing, and HiGHS says nothing at all until it finishes. Both are named under the chart rather than left out of it. This is also why the chart plots the gap and not the objective: the gap is a fraction, it runs from 100% down to a few ten-thousandths of a percent on a real model, and a logarithmic axis shows every decade. Objective values cannot be scaled that way — on a knapsack the opening bound is routinely twice the answer, and an axis wide enough for both flattens the entire search onto one line.
...
- **Which solver to pick by default** — a performance profile under the grid, once there are five datasets or more. For every dataset it divides each solver's time by the best time on that same dataset, then plots the share of datasets each solver finished within a given factor of the winner. The curve at 1x is how often that solver was outright fastest; where a curve flattens is the share of datasets it proved at all.

**The profile counts only a proven answer.** A run that hit the time limit holding a solution did not solve that dataset — it ran out of clock — so it never enters a ratio. Counting it would put the solver that gives up earliest at the top of the chart. A dataset no solver proved is dropped from every solver's denominator, because it separates nobody. And nothing is drawn below five datasets: on four, one dataset changing hands moves a curve a quarter of the chart, which reads as a difference between solvers when it is a difference of one run.
...
- A comparison of 5 solvers costs 5.
- A matrix of 3 datasets and 5 solvers costs **15** — the product, not the sum.
