...
/** Body accepted by POST /llm/conversations/{id}/explain-infeasibility. */
export interface ExplainInfeasibilityBody {
  execution_id?: string;
  formulation?: Record<string, unknown> | null;
  infeasibility?: Record<string, unknown> | null;
  use_advanced_model?: boolean;
}

export interface InfeasibilityExplanationState {
  ...
  explain: (conversationId: string, body: ExplainInfeasibilityBody) => Promise<void>;
  /** Cancel the current stream. */
  stop: () => void;
}

/**
 * POST-based SSE hook for the infeasibility-explanation endpoint.
 *
...
export function useInfeasibilityExplanation(): InfeasibilityExplanationState {
  ...
  const explain = useCallback(
    async (conversationId: string, body: ExplainInfeasibilityBody) => {
      if (abortControllerRef.current) {
        abortControllerRef.current.abort();
      }
      ...
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/api/v2/llm/conversations/${conversationId}/explain-infeasibility`,
          {
            method: "POST",
            headers,
