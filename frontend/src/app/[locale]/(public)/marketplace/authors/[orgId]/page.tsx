...
//
// `notFound` → real 404 (unknown/deleted author). `unavailable` (rate-limit
// 429 / 5xx / network) → still render a 200 with the client component loading
// the profile, rather than a spurious 404 that de-indexes a live author on a
...
const fetchOrgProfile = cache((orgId: string): Promise<SsrFetchResult<OrgProfileData>> => {
  return ssrJsonFetch<OrgProfileData>(`${apiUrl}/api/v2/organizations/${orgId}/public`, {
    label: "marketplace/authors/[orgId]",
  });
});

/** Return the URL only when it is a valid https:// URL. Guards author-supplied
...
export async function generateMetadata({
  ...
  if (result.status === "notFound") {
    return { title: "Author Profile | JAOT" };
  }
  ...
  if (result.status === "unavailable") {
    ...
    return buildPageMetadata({
      path: `/marketplace/authors/${orgId}`,
      locale,
  ...
  const description =
    ...
    t("dynamic.authorDescriptionFallback", { name: profile.name });
...
export default async function AuthorProfilePage({
  ...
  if (result.status === "notFound") {
    // Unknown / deleted author — return a real 404 instead of an empty 200 page
  ...
  if (result.status === "ok") {
    const profile = result.data;
    // Author-supplied website_url/logo_url are validated to https before inclusion;
    ...
    jsonLd = {
      ...
      url: safeHttpsUrl(profile.website_url) ?? `${BASE_URL}/marketplace/authors/${orgId}`,
      ...(logo ? { logo } : {}),
    };
  }
  ...
  return (
    <>
      ...
      <AuthorProfileClient orgId={orgId} />
    </>
  );
}
