...
export function OriginBadge({ origin, sourceKind, triggerName }: OriginBadgeProps) {
  ...
  // match the ORIGIN_STYLES keys, so a single typed t() call suffices. A studio
  // ModelProject wins over the looser origin slug the async solve carried.
  const resolved: OriginKey =
    sourceKind === "model_project"
      ? "model_project"
      : origin && origin in ORIGIN_STYLES
        ? (origin as OriginKey)
        : "manual";
  const title =
    resolved === "triggered"
